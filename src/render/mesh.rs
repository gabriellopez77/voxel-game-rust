...
impl Mesh {
    ...
    pub fn set<T>(&mut self, vertices: &[T], indices: &[u32], flags: BufferFlags) {
        self.update_or_realloc(BuffersTypes::Index, indices.len() * size_of::<u32>(), indices.as_ptr() as _, flags);
        self.update_or_realloc(BuffersTypes::Vertex, vertices.len() * size_of::<T>(), vertices.as_ptr() as _, flags);
    }
    ...
    pub fn update_instance_buffer<T>(&mut self, arr: &[T]) {
        if arr.len() > 0 {
            self.update_buffer(BuffersTypes::Instance, arr.len() * size_of::<T>(), arr.as_ptr() as _);
        }
    }
    ...
    fn update_or_realloc(&mut self, buffer_type: BuffersTypes, size: usize, data: *const u8, flags: BufferFlags) {
        ...
        if size > buffer.size || buffer.flags.contains(BufferFlags::ONCE) {
            ...
            self.update_buffer(buffer_type, size, data);
        }
    }

    fn update_buffer(&mut self, buffer_type: BuffersTypes, size: usize, data: *const u8) {
        if buffer_type == BuffersTypes::Index {
            self.triangles_count = (size / 4) as u32
        }

        if size > 0 {
            self.raw_buffers[buffer_type as usize].update(&mut self.app, size, 0, data);
        }
    }
}
